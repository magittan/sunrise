...
from PWE import NormalHedgeEnsemble
...
# exp name
exp_name = 'NH' + args.id + '_' + str(args.num_ensemble) + '/' + args.game
...
# Construct Ensembler for Action Selection
ensembler = NormalHedgeEnsemble(n_estimators=args.num_ensemble, loss_func=mean_absolute_error)
    ...
    for T in trange(1, args.T_max + 1):
        ...
        if args.ucb_infer > 0:
            ...
            for en_index in range(args.num_ensemble):
                ...
                if temp_count == 0:
                    var_Q = weights[en_index]*(L_target_Q[en_index] - mean_Q)**2
                else:
                    var_Q += weights[en_index]*(L_target_Q[en_index] - mean_Q)**2
                temp_count += 1
        ...
        if reward > 0:
            update_loss = np.ones(args.num_ensemble)
            Qs = np.array([dqn_list[en_index].get_online_q(state)[0][action].item() for en_index in range(args.num_ensemble)])
            loss_mask = Qs >= Qs[selected_index]
            update_loss[loss_mask] = 0
            ensembler.update(np.array([update_loss]).T,np.zeros(1))
        ...
        if T >= args.learn_start:
            ...
            if T % args.replay_frequency == 0:
                ...
                if args.temperature > 0:
                    ...
                    temp_count = 0
                    for en_index in range(args.num_ensemble):
                        e_weights = ensembler.weights.copy()
                        if temp_count == 0:
                            var_Q = e_weights[en_index]*(L_target_Q[en_index] - mean_Q)**2
                        else:
                            var_Q += e_weights[en_index]*(L_target_Q[en_index] - mean_Q)**2
                        temp_count += 1
                    var_Q = var_Q / temp_count
