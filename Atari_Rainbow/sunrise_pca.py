...
# VAE
encoder = pickle.load(open("{}_pca.pkl".format(args.game),'rb'))
kmeans_centroids = np.load('{}_centers.npy'.format(args.game))
...
# exp name
exp_name = 'PCA' + args.id + '_' + str(args.num_ensemble) + '/' + args.game
...
if args.evaluate:
    ...
    for T in trange(1, args.T_max + 1):
        ...
        # PCA + K-means weights
        mean_state = state.cpu().detach().numpy().sum(axis=0).ravel()
        encoded_obs = encoder.transform(np.reshape(mean_state,(1,84*84)))
        dists = np.array([np.linalg.norm(encoded_obs[0,:100] - cen) for cen in kmeans_centroids])
        pca_weights = softmax(dists - dists.mean())
        ...
        # Modify the Mask
        mask = torch.bernoulli(torch.Tensor(args.beta_mean*pca_weights))
        
        mem.append(state, action, reward, done, mask = mask)  # Append transition to memory
        # Train and test
